...
void HelloWorld::genBackground()
{
    ...
    if(terrain.stripes != NULL)
    {
        terrain.stripes->release();
        terrain.stripes = NULL;
    }
    
    CCSprite *stripes = this->stripedSpriteWithColor1(color3, color4, 512, 512, 4);
    ccTexParams tp2 = {GL_LINEAR, GL_LINEAR, GL_REPEAT, GL_CLAMP_TO_EDGE};
    stripes->getTexture()->setTexParameters(&tp2);
    stripes->retain();
    terrain.stripes = stripes;
}
